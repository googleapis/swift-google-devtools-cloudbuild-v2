...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-devtools-cloudbuild-v2.git --from 0.0.0-preview
```

Then add `GoogleCloudBuildV2` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudBuildV2 <target-name> --package swift-google-devtools-cloudbuild-v2
```
